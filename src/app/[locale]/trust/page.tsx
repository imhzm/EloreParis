...
export default async function Page({ params }: Props) { const { locale } = await params; if (!isLocale(locale)) notFound(); const schema = buildTrustSupportSchema(locale, "/trust", undefined, trustSlugs.map((slug) => ({ title: trustContent[locale][slug].title.replace("\n", " "), path: `/trust/${slug}` }))); return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><StorefrontShell activeHref="/trust" locale={locale} languageHref={`/${locale === "ar" ? "en" : "ar"}/trust`}><LocalizedTrustHub locale={locale} /></StorefrontShell></>; }
