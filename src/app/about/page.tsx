...
export const metadata: Metadata = { title: "قصتنا", description: "تعرّفي على منهج ÉLORÉ PARIS في الاختيار الهادئ والشرح الواضح.", alternates: { canonical: "/about" } };
export default function AboutPage() {
  const schema = { "@context": "https://schema.org", "@graph": [{ "@type": "AboutPage", name: "قصتنا", description: "صفحة تعريفية توضح منهج ÉLORÉ PARIS.", inLanguage: "ar-SA", url: absoluteUrl("/about") }, { "@type": "Organization", name: "ÉLORÉ PARIS", url: absoluteUrl("/") }, { "@type": "FAQPage", mainEntity: aboutFaq.map((faq) => ({ "@type": "Question", name: faq.question, acceptedAnswer: { "@type": "Answer", text: faq.answer } })) }] };
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><StorefrontShell activeHref="/about"><CinematicSupportStory kind="about" eyebrow="About ÉLORÉ PARIS" title="اختيار الجمال يحتاج إلى وضوح." intro="نبني تجربة تساعدك على فهم المنتج ومكانه داخل روتينك، قبل أن تطلب منك اتخاذ قرار الشراء." statement="نشرح أكثر، ونفترض أقل." principles={aboutHighlights} sectionTitle="هذه هي البنية التي نبني عليها الثقة." sections={aboutSections.map((section) => ({ title: section.heading, body: section.body }))} faqs={aboutFaq} nextHref="/trust" nextLabel="اكتشفي طبقات الثقة" /></StorefrontShell></>;
}
