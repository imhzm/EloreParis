...
export async function generateMetadata({params}:Props){const{locale}=await params;return isLocale(locale)?buildTrustSupportMetadata(locale,"/contact",supportContent[locale].contact):{};}
export default async function Page({params}:Props){const{locale}=await params;if(!isLocale(locale))notFound();const record=supportContent[locale].contact;return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(buildTrustSupportSchema(locale,"/contact",record))}}/><StorefrontShell activeHref="/contact" locale={locale} languageHref={`/${locale==="ar"?"en":"ar"}/contact`}><LocalizedTrustSupportDetail locale={locale} record={record} parentHref="/trust"/></StorefrontShell></>;}
