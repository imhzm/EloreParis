...
- Current focus: durable release-package publication history that preserves blockers, preflight, and latest evidence as a protected runtime trail before the first live Render deploy
...
- Published release packages are now stored durably inside the shared SQLite authority, exposed through `/api/ops/release/history`, rendered inside `/ops/release`, and exported from smoke plus live verification as release-history artifacts for review.
