...
- Current focus: owner-routed release blockers so `/ops/release` now shows who closes each remaining gate, which route they should use, and which next action still blocks a truthful hold or approve verdict against the current executive review state
...
- [x] Surface a structured delta between the current runtime and the package reviewed by the latest recorded decision
- [x] Route release blockers and runtime preflight checks to explicit owner lanes, owner routes, and next actions inside `/ops/release`, `/api/ops/release`, `/api/ops/release/package`, and `/api/ops/release/packet`
...
- Release governance now also stores durable blocker handoffs through `/api/ops/release/handoffs`, requires the latest executive packet plus full active-owner coverage for each handoff, and rejects protected release decisions until a current handoff exists for the latest packet.
