...
- Release decisions are now also bound to the latest executive packet review token and freshness window, so stale hold or approve submissions are rejected before they reach the durable governance trail.
...
14. Confirm `/api/ops/release/decisions` also rejects stale review tokens and expired review windows that are not based on the latest `/api/ops/release/packet`.
...
- `/api/ops/release/decisions` also refuses stale review tokens and expired review windows that are not based on the latest executive packet
