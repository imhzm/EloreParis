...
| SW-05 | Admin, catalog, data model, supplier ops, and order operations | `12-13` | `in-progress` | Design / Implementation | The admin scope, product data model, supplier sync assumptions, and order operations flow are explicitly designed, surfaced, and testable through protected internal routes, role-aware identity login, centralized SQLite-backed in-app authorities, and protected order, notification, and audit APIs |
...
- The new ops surfaces made gaps in auth, role separation, and persistent admin ownership more visible; a role-aware identity gate and local audit trail now exist for `/ops/*`, but provider-backed RBAC, durable shared audit storage, and backend ownership are still unresolved.
