...
- Release hardening has started through CI, manifest, fallback surfaces, a live `/ops/release` plus `/api/ops/release` readiness layer, and executable smoke-evidence artifacts, but deployment target credentials and monitoring are still undefined.
...
5. inject Vercel credentials, execute the first deployment, confirm `/ops/release` and `/api/ops/release/evidence` reflect the live environment honestly, and wire monitoring around the new `/api/health` endpoint
