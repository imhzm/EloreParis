...
- Release hardening has started through CI, manifest, fallback surfaces, a live `/ops/release` plus `/api/ops/release` readiness layer, executable smoke-evidence artifacts, and a manual Render deploy workflow that can publish verified live evidence back into the runtime after deploy, and the repository now freezes the primary host to a Render persistent runtime, but the first live service, domain binding, and monitoring are still external blockers.
...
5. create the first Render service from `render.yaml`, attach the persistent disk, configure the deploy-hook plus live-base-url secrets for the new manual workflow, confirm `/ops/release` and `/api/ops/release/evidence` reflect the live environment honestly, and wire monitoring around the new `/api/health` endpoint
